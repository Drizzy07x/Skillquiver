...
- Establish **self** — never "foreign": the skillquiver plugin, the skillquiver-dev plugin, and, when cwd is the Skillquiver repo, its `.claude/skills/`. Read the names from plugin.json, don't hardcode — forks stay excluded too.
...
| Stale skillquiver install | Old versions shipped different skills and can collide — suggest `claude plugin update skillquiver` before judging |
