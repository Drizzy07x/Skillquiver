...
1. **Resolve the filename the host loads.** Claude Code reads `CLAUDE.md`; Codex reads `AGENTS.md`. Write the file the running host actually loads. When `CLAUDE.md` is the target and `AGENTS.md` already holds canonical guidance, import it with `@AGENTS.md` instead of duplicating or symlinking it (symlinks need elevated rights on Windows). Keep `AGENTS.md` self-contained unless current Codex documentation confirms an import mechanism. Verify the file actually loads.
